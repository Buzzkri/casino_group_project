...
while play_again == 'y'
  ...
  def place_bet()
    ...
    # player_wins_losses_draws
    # if results[:wins] >= 1
    #   # new_total = @wallet.each.map { |num, i| + @money_bet[0]}
    #   # @wallet << new_total
    #   new_total =  @money_bet.first
    #   @wallet << new_total + @wallet.first
    # elsif results[:losses] >= 1
    #   new_total = @wallet.first - @money_bet.first
    #   @wallet << new_total
